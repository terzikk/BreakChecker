...
def save_results(results: dict, domain: str, json_output: bool = False) -> None:
    ...
    if json_output:
        ...
        if crawler:
            write_map(os.path.join(output_dir, "email_sources.txt"), crawler.emails, email_sources)
            write_map(os.path.join(output_dir, "phone_sources.txt"), crawler.phones, phone_sources)
        write_list(os.path.join(output_dir, "emails.txt"), emails)
        write_list(os.path.join(output_dir, "phones.txt"), phones)
        write_list(os.path.join(output_dir, "breached_emails.txt"), breached_emails.keys())
...
async def scan_domain(
    ...
    *,
    ...
    if verbose:
        logger.debug(
            "%d of %d subdomains are accessible", len(subdomain_schemes), len(subs)
        )
    ...
    crawler = Crawler(domain, max_depth=0 if use_katana else depth, concurrency=concurrency)
...
async def main() -> dict:
    ...
    cfg = load_config()
    default_depth = int(os.environ.get("CRAWL_DEPTH", cfg.get("crawl_depth", 3)))
